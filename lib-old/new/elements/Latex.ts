/**
 * NewLatex - Pure LaTeX formula rendering for the new layout system.
 *
...
/**
 * Represents an annotated element within a NewLatex formula.
 * Annotated using \cssId{id}{content} or \class{class}{content} commands.
 */
export interface NewAnnotatedLatexElement {
...
/**
 * Configuration for creating a NewLatex element.
 */
export interface NewLatexConfig {
  /**
 ...
 *
 * The NewLatex class renders mathematical notation using MathJax and provides
 * measurement capabilities and annotation support for highlighting specific parts.
 *
 * @example
 * Create a simple formula
 * ```typescript
 * const formula = new NewLatex({
 *   content: "E = mc^2",
 *   fontSize: 24
 * });
 * ```
 ...
 * ```
 */
export class NewLatex extends NewShape {
  private config: NewLatexConfig;
  ...
  /**
   * Creates a new NewLatex instance.
   *
   * @param config - Configuration for the LaTeX element
   */
  constructor(config: NewLatexConfig) {
    super();
    this.config = config;
    this._fontSize =
   ...
   * ```
   */
  getElementById(id: string): NewAnnotatedLatexElement | null {
    this.ensureMeasured();
   ...
   * ```
   */
  getElementsByClass(className: string): NewAnnotatedLatexElement[] {
    this.ensureMeasured();
    ...
    try {
      ...
      const results: NewAnnotatedLatexElement[] = [];
